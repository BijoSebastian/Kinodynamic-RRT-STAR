...
def main():
    ...
    while len(nodes) < NUMNODES:
        ...
        flagg = 0 #to check if the random sample was of use
        nn = nodes[0] #nn is nearest node to rand_n                
        for p in nodes:            
            
            if dist(p, rand_n) < RADIUS:                
                if not checkIntersect(p, rand_n):
                    if len(nodes) == 1: #case where there is only one node
                        flagc = 0 #Meaning no collission happened
                    else:
                        if p.cost+ dist(p, rand_n) < nn.cost+dist(nn, rand_n):
                            nn = p
                            flagc = 0 #Meaning no collission happened
       
        if flagc == 0:
            rand_n.cost = nn.cost + dist(nn, rand_n)
            rand_n.parent = nn            
            flagg = 1
        
        else:#No collsion free nodes so step                                                             
            nn = nodes[0]
            for p in nodes:                                                
                if p.cost+ dist(p, rand_n) < nn.cost+dist(nn, rand_n):                    
                    nn = p
            ...
            if rand_n != nn: #Meaninng we were able to step towards the random sample atleast once                
                rand_n.cost = nn.cost + dist(nn, rand_n)
                rand_n.parent = nn                
                flagg = 1               
        
        if  flagg == 0: #This particular random sample was of no use
            continue 
                           
                                                    
        #Append and rewire        
        nodes.append(rand_n)
        print ("newnode", rand_n.x, rand_n.y) 
        plt.plot(rand_n.x, rand_n.y, 'bo', ms = 4.0)                
        nodes = reWire(nodes, rand_n) 
