...
# cubeStartPos = [0.0,0.0,0.14]#[5, 3, 0.75]
...
# PBT.setJointMotorControl2(boxId, 0, PBT.POSITION_CONTROL, targetPosition = -50, force = 700)
# PBT.setJointMotorControl2(boxId, 1, PBT.POSITION_CONTROL, targetPosition = -50, force = 350)
# for _ in range(200):
#     PBT.stepSimulation()
#     time.sleep(.000001)
# input("check")     
...
def pub_vel(v, w, theta1, theta2):    
    ...
    #Set theta1 
    PBT.setJointMotorControl2(boxId, 0, PBT.POSITION_CONTROL, targetPosition = theta1, force = 500)
     
    #Set theta2  
    PBT.setJointMotorControl2(boxId, 1, PBT.POSITION_CONTROL, targetPosition = theta2, force = 350)
...
#         if cubePos[2] > 0.7:
#             print ("hi")
#             print(PBT.getBasePositionAndOrientation(boxId))
#             input()  
#         time.sleep(0.01)           
    return
...
def pose_update():
    ...
    temp = PBT.getJointState(boxId, 0)
    theta1 = temp[0]
    temp = PBT.getJointState(boxId, 1)
    theta2 = temp[0]
    pose = [cubePos[0], cubePos[1], cubeOrn[2], theta1, theta2]    
    return
...
def at_goal():
    ...
    #check if angles are near
    a = np.sqrt(pow((goal[3] - pose[3]),2) + pow((goal[4] - pose[4]),2))
    
    if d <= distThresh and a <= angleThresh :
        #print ("Reached goal")
        return True
    else:
        return False
...
def checkIntersect(p1, p2):
    ...
    if [p1.x, p1.y, p1.theta, p1.theta1, p1.theta2, p2.x, p2.y, p2.theta, p2.theta1, p2.theta2] in phydatabase1:
        #print ("true", p1.x, p1.y, p2.x, p2.y)
        return phydatabase2[phydatabase1.index([p1.x, p1.y, p1.theta, p1.theta1, p1.theta2, p2.x, p2.y, p2.theta, p2.theta1, p2.theta2])]
    
    #Import HMMR at desired pos
    cubeStartPos = [p1.x, p1.y, 0.14, ]
    cubeStartOrientation = PBT.getQuaternionFromEuler([0.0, 0.0, p1.theta])
    boxId = PBT.loadURDF("HMMR/urdf/HMMR.urdf",cubeStartPos, cubeStartOrientation)
    PBT.setJointMotorControl2(boxId, 0, PBT.POSITION_CONTROL, targetPosition = p1.theta1)
    PBT.setJointMotorControl2(boxId, 1, PBT.POSITION_CONTROL, targetPosition = p1.theta2)
    ...
    pub_vel(0.0, 0.0, p2.theta1, p2.theta2)
    PBT.removeBody(boxId) 
    #print (temp_cost)
     
    if temp_cost< 100:
        phydatabase1.append([p1.x, p1.y, p1.theta, p1.theta1, p1.theta2, p2.x, p2.y, p2.theta, p2.theta1, p2.theta2])
...
def drawSolutionPath(start_pos, goal_pos, nn):    
    ...
    boxId = PBT.loadURDF("HMMR/urdf/HMMR.urdf",cubeStartPos, cubeStartOrientation)
    PBT.setJointMotorControl2(boxId, 0, PBT.POSITION_CONTROL, targetPosition = start_pos.theta1)
    PBT.setJointMotorControl2(boxId, 1, PBT.POSITION_CONTROL, targetPosition = start_pos.theta2)
    ...
    for i in range(len(solution)):
        ...
        while  not at_goal():          
            ...
            pose_update()  
            time.sleep(0.1)    
        pub_vel(0.0, 0.0, solution[i].theta1, solution[i].theta2)
...
def main():
    ...
    goal_pos = Node(5.0, 3.0, 0.0, 0.0, 0.0) # These needs to be better defined probably as functions
    ...
    while len(nodes) < NUMNODES:
        print (len(nodes))
        random.seed()
        rand_n = Node(random.random()*Window_size, random.random()*Window_size, 0.0, random.random()*Window_size, random.random()*Window_size) #rand_n is randomly sampled node        
        print ("rand", rand_n.x, rand_n.y, rand_n.theta, rand_n.theta1, rand_n.theta2)
        ...
        if flagc == 1:
            ...
            print ("step", p.x, p.y, p.theta, p.theta1, p.theta2)
            print ("rand_n", rand_n.x, rand_n.y, rand_n.theta, rand_n.theta1, rand_n.theta2)
        ...
        print ("newnode", newnode.x, newnode.y, newnode.theta, newnode.theta1, newnode.theta2) 
...
#The one way to get rid of all kinds of collisoins would be to actually go in and do contact checks
#But that will be an issue in climbing since that has contacts
