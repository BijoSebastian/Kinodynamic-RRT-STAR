...
from matplotlib.tri.triinterpolate import _transpose_vectorized
from zipfile import _FH_GENERAL_PURPOSE_FLAG_BITS
...
physicsClient = PBT.connect(PBT.DIRECT)

#Set gravity
PBT.setGravity(0,0,-10)

#Import ground plane
planeId = PBT.loadURDF("plane.urdf")

# #Import obstacle box
# obsboxStartPos = [5,0,0.3]
# obsboxStartOrientation = PBT.getQuaternionFromEuler([0,0,0])
# obsId = PBT.loadURDF("box/urdf/box.urdf",obsboxStartPos, obsboxStartOrientation)
...
def drawSolutionPath(start, goal, nodes):    
    ...
    for p in nodes:
        ...
        if dist(p, goal) < dist(nn, goal):
            nn = p            
    while nn!=start:
        print (nn.x, nn.y, nn.theta, nn.theta1, nn.theta2)
...
def main():
    ...
    goal = Node(50.0, 20.0, 0.0, 0.0, 0.0) # These needs to be better defined probably as functions
        ...
        for p in nodes:                                                
            if dist(p, rand_n) <= dist(nn, rand_n):
                if not checkIntersect(p, rand_n):
                    nn = p
                    flagc = 0 #Meaning no colliosn happened
